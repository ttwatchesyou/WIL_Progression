...
export default function TeacherDashboard() {
  ...
  const [attendanceMap, setAttendanceMap] = useState<{ [key: number]: 'present' | 'late' | 'absent' | 'leave' }>({});
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [studentsRes, tasksRes, reportsRes, promoRes]: any = await Promise.all([
      ...
      if (studentsRes?.data) {
        ...
        const initialMap: { [key: number]: 'present' | 'late' | 'absent' | 'leave' } = {};
  ...
  const loadExistingAttendance = async () => {
    try {
      ...
      const res: any = await attendanceService.getAttendance({ date: formattedDate });
  ...
  const handleAttendanceChange = (studentId: number, status: 'present' | 'late' | 'absent' | 'leave') => {
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      message.success(`บันทึกการเช็คชื่อวันที่ ${attendanceDate.format('DD/MM/YYYY')} เรียบร้อยแล้ว`);
    } catch (error: any) {
  ...
  const handleApprove = async (taskId: number, step: 'step1_assign' | 'step2_grade', status: 'approved' | 'rejected') => {
  ...
  const handleCreateTask = async (values: any) => {
    ...
    try {
      ...
      message.success(`สั่งงานแก่นักเรียน ${values.student_ids.length} คนเรียบร้อยแล้ว!`);
  ...
  const classroomsList = Array.from(new Set(students.map((s) => s.classroom).filter(Boolean)));
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Radio.Group
          ...
          size="small"
        >
          <Radio.Button value="present" style={{ color: attendanceMap[record.id] === 'present' ? '#fff' : '#059669' }}>
            มา
          </Radio.Button>
  ...
  const taskColumns = [
    {
      ...
      render: (text: string, record: any) => (
        <div>
          ...
          {record.description && (
            <div style={{ fontSize: '0.78rem', color: '#64748b' }}>{record.description}</div>
          )}
        </div>
      ),
    },
    {
      ...
      render: (id: number) => {
        ...
        return student ? (
          <Tag color="blue">{student.first_name} {student.last_name}</Tag>
        ) : (
  ...
  const promotionColumns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <span>
          <Tag color="blue">Lv.{record.current_rank}</Tag> ➔ <Tag color="gold">Lv.{record.proposed_rank}</Tag>
        </span>
      ),
    },
    {
  ...
  const displayStudents = selectedClassroom === 'all'
    ? students
    : students.filter((s) => s.classroom === selectedClassroom);

  return (
    <MainLayout userName={currentUser ? `${currentUser.first_name || ''} ${currentUser.last_name || ''}` : 'อาจารย์'} rankLevel={99}>
      {loading ? (
        ...
        <DashboardContainer>
          <WelcomeBanner>
            ...
            <ActionButtonGroup>
              <Button
                ...
                style={{ background: 'rgba(255,255,255,0.15)', color: '#fff', border: '1px solid #d4af37', fontWeight: 600, borderRadius: 12 }}
              ...
              <Button
                ...
                size="large"
                icon={<PlusOutlined />}
                style={{ background: '#d4af37', color: '#0a192f', border: 'none', fontWeight: 700, borderRadius: 12 }}
            ...
            <Tabs
              ...
              items={[
                  ...
                  children: (
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', width: '100%', maxWidth: 400 }}>
                          <DatePicker
                        ...
                        <Button
                          ...
                          style={{ background: '#0a192f', border: '1px solid #d4af37', borderRadius: 10 }}
                          ...
                          displayStudents.map((student) => (
                            <MobileStudentCard key={student.id}>
                              <StudentHeaderRow>
                                ...
                                <Tag color="blue">{student.classroom || 'ทั่วไป'}</Tag>
                              </StudentHeaderRow>

                              <StudentMetaTag>
                                รหัสนักศึกษา: {student.student_code || student.username}
                              </StudentMetaTag>
                              ...
                              <TouchRadioGroup
                                ...
                                buttonStyle="solid"
                              >
                                <Radio.Button value="present" style={{ color: attendanceMap[student.id] === 'present' ? '#fff' : '#059669' }}>
                                  มา
                                </Radio.Button>
                                <Radio.Button value="late">สาย</Radio.Button>
                                <Radio.Button value="leave">ลา</Radio.Button>
                ...
                },
                {
                  ...
                  label: (
                    <span>
                      <CheckCircleOutlined /> ตรวจงาน ({pendingStep1Count + pendingStep2Count})
                    </span>
                  ),
                  children: (
                    <StyledTable
                        ...
                        reports.map((item: any) => {
                          ...
                          return (
                            <Col xs={24} sm={12} md={8} key={item.id}>
                              ...
                              >
                                <div style={{ fontWeight: 700, color: '#0a192f', marginBottom: 2 }}>
                                  {item.first_name} {item.last_name} ({item.student_code})
                                </div>
                                <div style={{ fontSize: 11, color: '#64748b', marginBottom: 8 }}>
                                  วันที่: {item.report_date}
                                </div>
                                <p style={{ fontSize: 12, color: '#334155', margin: 0 }}>{item.details}</p>
                                    ...
                                    <Image.PreviewGroup>
                                      {images.map((imgUrl, idx) => (
                                        <Image
                                          ...
                                          src={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}${imgUrl}`}
                                          alt="รูปปฏิบัติงาน"
                                        />
                                      ))}
                                    </Image.PreviewGroup>
          ...
          >
            <Form form={taskForm} layout="vertical" onFinish={handleCreateTask}>
              <Form.Item
                ...
                name="student_ids"
                rules={[{ required: true, message: 'กรุณาเลือกนักเรียนอย่างน้อย 1 คน' }]}
              >
                <Select
                  mode="multiple"
                  ...
                  options={students.map((s) => ({
                    value: s.id,
                    label: `${s.first_name} ${s.last_name} (${s.student_code || s.username})`,
                  }))}
                />
              </Form.Item>
              ...
              >
                <Select placeholder="เลือกทักษะ" size="middle">
                  <Select.Option value="PLC Programming">PLC Programming</Select.Option>
                  <Select.Option value="Pneumatics System">Pneumatics System</Select.Option>
                  <Select.Option value="Robotics Control">Robotics Control</Select.Option>
                  <Select.Option value="Electrical Circuit">Electrical Circuit</Select.Option>
                  <Select.Option value="Sensor & Actuator">Sensor & Actuator</Select.Option>
                </Select>
              </Form.Item>
          ...
          <Modal
            ...
            width={480}
          >
            <Form form={promotionForm} layout="vertical" onFinish={handleSendPromotion}>
              <Form.Item label="เลือกนักเรียน" name="student_id" rules={[{ required: true }]}>
                <Select
                  placeholder="เลือกนักเรียน..."
                  optionFilterProp="label"
                  size="middle"
                  options={students.map((s) => ({
                    value: s.id,
                    label: `${s.first_name} ${s.last_name} (ปัจจุบัน Rank Lv.${s.rank_level || 1})`,
                  }))}
                />
              </Form.Item>

              <Form.Item label="Rank Level ที่เสนอขึ้นไป" name="proposed_rank" rules={[{ required: true }]}>
                <InputNumber min={2} max={10} style={{ width: '100%' }} size="middle" placeholder="เช่น Rank 3" />
              </Form.Item>

              <Form.Item label="เหตุผล / ผลงานโดดเด่น" name="reason">
                <Input.TextArea rows={3} placeholder="อธิบายผลงานที่สมควรได้รับการเลื่อนขั้น..." />
              </Form.Item>

              <Button type="primary" htmlType="submit" icon={<SendOutlined />} block style={{ background: '#0a192f', border: '1px solid #d4af37', borderRadius: 10, height: 42 }}>
                ส่งเสนอแอดมิน
              </Button>
            </Form>
          </Modal>
